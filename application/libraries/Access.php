 ...
 */
class CI_Access extends CI_Controller {
	...
	*/
	...
	public function __construct($params = array())
	{
		...
		$this->controller_segment = $this->CI->uri->rsegments[$this->uri_segment];
		$this->function_segment = $this->CI->uri->rsegments[$this->uri_segment+1];
	...
	function initialize()
	{
		...
		//{
		//	redirect($this->CI->config->base_url().$this->login_path."?".$this->redirect_query_string."=".$this->request_uri);
			//die();
		//}
		//user will be directing to base_url if he was access login page
		if($this->is_logged() && $this->request_page == $this->login_path)
		{
			redirect($this->CI->config->base_url());
			die();
		}
		$this->session->userdata[$this->session_access_name] = array('welcome'=> array('index'));//;
		//set access function
		...
		$query = $this->CI->db->get('menu')->result();
		//print_r($query);
		$access_id = array();
